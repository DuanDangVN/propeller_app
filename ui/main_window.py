"""Main application window and UI event handling."""
...
from PySide6.QtCore import QEventLoop, QThread, QTimer, QUrl, Qt, QSize, Signal
...
from app_runtime import (
    APP_CONFIG_PATH,
    CALIBRATION_CAPTURE_WINDOW_S,
    CALIBRATION_MIN_DELTA_V,
    CALIBRATION_PATH,
    EXPORT_DIR,
    PUBLIC_DIR,
    RPM_SENSOR_PROFILE,
    smoke_trace as _smoke_trace,
)
from devices import (
...
class MainWindow(QMainWindow):
    ...
    def __init__(self):
        ...
        analog_config = {
            ...
            "terminal_configuration": "RSE",
            "analog_reference": "AI GND",
        }
        acquisition_config.update(analog_config)
        acquisition_config.pop("force_negative_input", None)
        acquisition_config.pop("torque_negative_input", None)
        filter_config = self.app_config["filter"]
        display_config = self.app_config["display"]
        rpm_config = self.app_config.setdefault("rpm", {})
        if rpm_config.get("sensor_profile") != RPM_SENSOR_PROFILE:
            previous_ppr = rpm_config.get("pulses_per_revolution")
            rpm_config["sensor_profile"] = RPM_SENSOR_PROFILE
            rpm_config["pulses_per_revolution"] = (
                DEFAULT_PULSES_PER_REVOLUTION
            )
            with APP_CONFIG_PATH.open(
                "w",
            ...
            LOGGER.info(
                "Migrated RPM sensor profile from ppr=%s to ppr=%s",
                previous_ppr,
                DEFAULT_PULSES_PER_REVOLUTION,
            )
    ...
    def _setup_tab1(
        ...
        acquisition_mode_label = QLabel(
            ...
            "Analog: Force AI0, Torque AI1, RSE referenced to AI GND"
        )
        ...
        self.calibration_parameter = QComboBox()
        self.calibration_parameter.setSizePolicy(
            QSizePolicy.Expanding,
            QSizePolicy.Expanding,
        )
        #cablicate_parameter.setFixedHeight(text_input_height)
        self.calibration_parameter.addItems(["None", "Thrust", "Torque"])
        self.calibration_parameter.currentTextChanged.connect(
            self.handle_parameter_selection
        )
        col2_layout.addWidget(self.calibration_parameter, 1)
        # -- -- -- Input Mass
        ...
        self.input_mass.setPlaceholderText("Mass in grams, e.g. 2000 for 2 kg")
        grid1_layout.addWidget(self.input_mass, 1, 1)
        # -- -- -- Get Voltage
        self.get_voltage_click_count = 0
        self.get_vol = QPushButton("Get Voltage (0)")
        ...
        self.show_voltage = QLabel("Live voltage: select Thrust or Torque")
        ...
        # -- -- -- Show storage value
        self.show_storage = QLabel("Clicks: 0 | Captured points: 0")
        ...
        self.view_applied_calibration = QPushButton(
            "View applied calibration"
        )
        self.view_applied_calibration.setSizePolicy(
            QSizePolicy.Expanding,
            QSizePolicy.Fixed,
        )
        self.view_applied_calibration.setFixedHeight(text_input_height)
        self.view_applied_calibration.clicked.connect(
            self.show_applied_calibration
        )
        grid1_layout.addWidget(self.view_applied_calibration, 4, 1, 1, 3)
        # --
    ...
    def apply_processing_settings(self):
        ...
        try:
            ...
            self.app_config["rpm"] = {
                ...
                "sensor_profile": RPM_SENSOR_PROFILE,
    ...
    @staticmethod
    def _format_applied_calibration(
        name,
        unit,
        calibration,
    ):
        """Format the exact equation currently used for one sensor."""
        zero_offset_v = calibration.zero_offset_v
        if zero_offset_v is None:
            zero_text = "Not set"
            equation = (
                f"{name} ({unit}) = {calibration.slope:.9g} × V "
                f"{calibration.intercept:+.9g}"
            )
        else:
            zero_text = f"{zero_offset_v:.9g} V"
            equation = (
                f"{name} ({unit}) = {calibration.slope:.9g} × "
                f"(V - {zero_offset_v:.9g})"
            )
        return (
            f"{name}\n"
            f"A (slope): {calibration.slope:.9g}\n"
            f"B (intercept): {calibration.intercept:.9g}\n"
            f"Zero: {zero_text}\n"
            f"Applied formula: {equation}"
        )

    def _applied_calibration_text(self):
        """Return both force and torque calibration summaries."""
        thrust = self._format_applied_calibration(
            "Thrust",
            "N",
            self.force_calibration,
        )
        torque = self._format_applied_calibration(
            "Torque",
            "N.m",
            self.torque_calibration,
        )
        return f"{thrust}\n\n{torque}"

    def show_applied_calibration(self):
        """Show the calibration equations currently used for measurement."""
        QMessageBox.information(
            self,
            "Applied calibration",
            self._applied_calibration_text(),
        )
    ...
    def set_calib_slope_value(self):
        try:
            ...
            if not hasattr(self, "slope") or not hasattr(self, "intercept"):
                raise ValueError("Run Linear approximate before applying.")
            ...
            QMessageBox.information(
                self,
                "Success",
                f"{self.parameter_selected} is calibrated: "
                f"slope A = {self.slope}, intercept B = {self.intercept}\n"
                "Run Zero again with no load before measuring.",
            )
        except Exception as e:
    ...
    def linear_data_value(self):
        try:
            if len(self.vol_cablibration) < 2:
                raise ValueError("Capture at least two calibration loads.")
            voltage_span = float(np.ptp(self.vol_cablibration))
            if voltage_span < CALIBRATION_MIN_DELTA_V:
                raise ValueError(
                    "Calibration voltage did not change enough. Check the "
                    "selected channel, JSY-S60 wiring and applied load."
                )
    ...
    def handle_parameter_selection(self, selected_text):
        previous_parameter = self.parameter_selected
        if (
            previous_parameter != selected_text
            and previous_parameter in ("Thrust", "Torque")
        ):
            self.clear_all_value()
        self.parameter_selected = selected_text
        self._update_calibration_voltage_display()

    def clear_all_value(self):
        self.mass_cablibration = []
        self.vol_cablibration = []
        for fit_attribute in ("slope", "intercept"):
            if hasattr(self, fit_attribute):
                delattr(self, fit_attribute)
        self.get_voltage_click_count = 0
        self.get_vol.setText("Get Voltage (0)")
        self.plot_widget.clear()
        self.show_storage.setText("Clicks: 0 | Captured points: 0")
        self.show_slope.setText("No linear fit")

    def _calibration_samples(self):
        """Return the live samples and NI channel for the selected parameter."""
        if self.parameter_selected == "Thrust":
            return self.recent_force_voltage, FORCE_CHANNEL
        if self.parameter_selected == "Torque":
            return self.recent_torque_voltage, TORQUE_CHANNEL
        raise ValueError("Select Thrust or Torque first.")

    def _update_calibration_voltage_display(self):
        """Show a live raw-voltage diagnostic while the main reader is active."""
        if not hasattr(self, "show_voltage"):
            return
        if self.parameter_selected not in ("Thrust", "Torque"):
            self.show_voltage.setText("Live voltage: select Thrust or Torque")
            return
        samples, _channel = self._calibration_samples()
        if samples.size == 0:
            self.show_voltage.setText(
                f"Live {self.parameter_selected}: waiting for NI samples"
            )
            return
        try:
            summary = summarize_voltage_samples(
                samples,
                self.sampling_rate,
                CALIBRATION_CAPTURE_WINDOW_S,
            )
            self.show_voltage.setText(
                f"Live {self.parameter_selected}: {summary.mean_v:.6f} V "
                f"(σ {summary.standard_deviation_v:.5f} V)"
            )
        except ValueError as exc:
            self.show_voltage.setText(f"Voltage unavailable: {exc}")

    def get_voltage_value(self):
        """Capture calibration data without blocking the GUI thread."""
        try:
            if self.parameter_selected not in ("Thrust", "Torque"):
                raise ValueError("Select Thrust or Torque first.")
            if self.calibration_thread is not None:
                raise RuntimeError("A calibration capture is already running.")
            mass_text = self.input_mass.text().strip().replace(",", ".")
            ...
            if self.read_sensor_running:
                if (
                    self.recent_force_voltage.size == 0
                    or self.recent_torque_voltage.size == 0
                ):
                    raise RuntimeError(
                        "NI reading has started but no analog samples have "
                        "arrived yet."
                    )
                self._complete_calibration_capture(
                    self.recent_force_voltage,
                    self.recent_torque_voltage,
                )
                return

            if self.acquisition_mode == "hardware" and not self.dev_selected:
                raise RuntimeError(
                    "No NI device is selected. Connect USB-6001 and click "
                    "Scan NI devices."
                )

            sample_count = max(
                10,
                int(round(self.sampling_rate * CALIBRATION_CAPTURE_WINDOW_S)),
            )
            chunk_size = min(
                sample_count,
                max(1, int(round(self.sampling_rate * 0.1))),
            )
            reader_factory = (
                SimulationReader
    ...
    def _set_calibration_controls_enabled(self, enabled):
        """Prevent calibration state changes during a finite capture."""
        for control in (
            self.calibration_parameter,
            self.get_vol,
            self.clear_all,
            self.linear_data,
            self.set_calib_value,
        ):
            control.setEnabled(bool(enabled))

    def _update_calibration_progress(self, collected, total):
        """Display progress for an independent calibration capture."""
        self.show_storage.setText(
            f"Samples: {int(collected)}/{int(total)} | "
            f"Captured points: {len(self.vol_cablibration)}"
        )

    def _complete_calibration_capture(self, force_voltage, torque_voltage):
        """Validate and store one averaged calibration point."""
        try:
            parameter = self.pending_calibration_parameter
            mass = self.pending_calibration_mass
            if parameter not in ("Thrust", "Torque") or mass is None:
                raise RuntimeError("Calibration request state was lost.")
            ...
            if sample_count <= 0:
                raise RuntimeError("Calibration returned no analog samples.")
            samples = (
                force_voltage[:sample_count]
                if parameter == "Thrust"
                else torque_voltage[:sample_count]
            )
            summary = summarize_voltage_samples(
                samples,
                self.sampling_rate,
            ...
            mean_vol = summary.mean_v
            self.title_plot = (
                "Thrust (N)" if parameter == "Thrust" else "Torque (N.m)"
            )
            converted_mass = calibration_value_from_mass(mass, parameter)

            if self.vol_cablibration:
                value_changed = not np.isclose(
                    converted_mass,
                    self.mass_cablibration[-1],
                    rtol=0.0,
                    atol=1e-12,
                )
                voltage_delta = abs(mean_vol - self.vol_cablibration[-1])
                if value_changed and voltage_delta < CALIBRATION_MIN_DELTA_V:
                    QMessageBox.warning(
                        self,
                        "Calibration signal did not change",
                        f"The applied load changed, but voltage changed only "
                        f"{voltage_delta:.6f} V. This point was not stored.\n\n"
                        "Check: selected Thrust/Torque channel, JSY-S60 output "
                        "wiring, AI+/AI- polarity, and wait for the load to settle.",
                    )
                    return
            self.vol_cablibration.append(mean_vol)
            self.mass_cablibration.append(converted_mass)
            self.get_voltage_click_count += 1
            self.get_vol.setText(
                f"Get Voltage ({self.get_voltage_click_count})"
            )
            self.show_voltage.setText(
                f"Captured {parameter}: {mean_vol:.6f} V "
                f"(std {summary.standard_deviation_v:.5f} V)"
            )
            self.show_storage.setText(
                f"Clicks: {self.get_voltage_click_count} | "
                f"Captured: {len(self.vol_cablibration)} | Last: {mass:g} g -> "
                f"{mean_vol:.6f} V"
            )
            LOGGER.info(
                "Calibration point captured: parameter=%s samples=%s "
                "voltage=%.9f",
                parameter,
            ...
            self._on_calibration_error(str(exc))
        finally:
            self.pending_calibration_mass = None
            self.pending_calibration_parameter = None
    ...
    def _on_calibration_thread_finished(self):
        """Release references after a finite calibration capture."""
        self.calibration_worker = None
        self.calibration_thread = None
        self._set_calibration_controls_enabled(True)

    def stop_calibration_capture(self):
        """Stop a temporary calibration worker during application shutdown."""
        ...
        if thread.isRunning() and worker is not None:
            ...
            if thread.isRunning():
                LOGGER.error(
                    "Calibration worker did not stop within 3 seconds"
                )
    ...
    def stop_reading(self):
        ...
        if thread is not None and thread.isRunning():
            ...
            self.request_acquisition_stop.emit()
        # Qt automatically removes signal connections when the worker is
        # deleted. Explicitly disconnecting here can race with deleteLater()
        # after the acquisition thread has already finished.
        self.acquisition_worker = None
        self.acquisition_thread = None
    ...
    def get_data(self, vol_thrust, vol_torque, rpm_reading):
        ...
        rpm_elapsed_s, self.rpm_time_offset_s = align_rpm_elapsed_time(
            elapsed_s,
            rpm_reading.elapsed_time_s,
            self.rpm_time_offset_s,
        )
        self.update_revolution(rpm_reading, rpm_elapsed_s)
    ...
    def _zero_channels(self):
        ...
        try:
            ...
            if (
                self.recent_force_voltage.size < required
                or self.recent_torque_voltage.size < required
            ):
                raise RuntimeError(
    ...
    def save_excel_data(self):
        """Save the three primary measurements to Excel."""
